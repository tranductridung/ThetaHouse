      ...
      <div className="p-4">
        ...
        <div className="flex flex-1 flex-col md:flex-row pt-5 rounded-xl text-gray-500 shadow-xl text-md md:text-xl ">
          <div className="flex-1/3 grid md:grid-rows-3 p-5">
            <div className="flex flex-col py-3 space-y-3 w-full bconsignment-b-2">
              <h1 className="font-bold text-black">Creator</h1>
              <div className="flex flex-row space-x-3">
                <User />
                <span>{consignment?.creator.fullName}</span>
              </div>

              <div className="flex flex-row space-x-3">
                <Mail /> <span>{consignment?.creator.email}</span>
              </div>
              <div className="flex flex-row space-x-3">
                <Phone />
                <span>{consignment?.creator.phoneNumber}</span>
              </div>
            </div>

            <div className="py-3 w-full bconsignment-b-2">
              <h1 className="font-bold text-black">Note</h1>
              <span>{consignment?.note ?? "No note"}</span>
            </div>
          </div>

          <div className="flex flex-1/3 flex-col p-5 space-y-5">
            <h1 className="font-bold text-black">Consignment Summary</h1>
            <div className="flex flex-row justify-between">
              <h1>Items: </h1>
              <p>{consignment?.items.length}</p>
            </div>

            <div className="flex flex-row justify-between">
              <h1>Quantity: </h1>
              <p>{consignment?.quantity}</p>
            </div>

            <div className="flex flex-row justify-between ">
              <h1>Subtotal: </h1>
              <p>{consignment?.totalAmount}</p>
            </div>

            <div className="flex flex-row justify-between">
              <h1>Discount: </h1>
              <p>{consignment?.discount ?? "No Discount"}</p>
            </div>

            <div className="flex flex-row justify-between font-bold bconsignment-t-2 text-black">
              <h1>Total: </h1>
