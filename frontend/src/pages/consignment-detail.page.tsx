...
const ConsignmentDetails = ({ isUseTitle = true }: ConsignmentDetailsProps) => {
  ...
  useEffect(() => {
    const run = async () => {
      try {
        setLoading(true);
        await fetchPermissions(["consignment", "item", "payment"]);
        await fetchData();
      } finally {
        setLoading(false);
      }
  ...
  return (
    <>
      <RequirePermission permission="consignment:read">
        <div className="p-4">
          ...
          <div className="flex flex-col space-y-5">
            <RequirePermission permission="item:read">
              <DataTable
                ...
                onAdd={
                  consignment?.status !== "Cancelled" ? onAddItem : undefined
                }
                ...
                title={"Add Item"}
                permission={
                  consignment?.status !== "Cancelled" ? "item:create" : ""
                }
              ></DataTable>
            </RequirePermission>
            ...
            <RequirePermission permission="payment:read">
              <DataTable
                ...
                title={"Add Payment"}
                permission={
                  consignment?.status !== "Cancelled" ? "payment:create" : ""
                }
              ></DataTable>
            </RequirePermission>
          </div>
