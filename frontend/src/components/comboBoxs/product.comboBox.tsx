...
export function ProductComboBox({
  ...
  value: ProductOption | null | undefined;
  ...
  useEffect(() => {
    ...
    const fetchProducts = async () => {
      ...
      try {
        const url = `/products?page=${page}&limit=${pageSize}&search=${input}`;

        const res = await api.get(url);
        const items: ProductOption[] = res.data.products || [];
  ...
  useEffect(() => {
    if (value && open) {
      const exists = products.some((p) => p.id === value.id);
      if (!exists) {
        setProducts((prev) => [value, ...prev]);
      }
    }
  }, [value, open, products]);
